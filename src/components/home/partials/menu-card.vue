<template>
  <div class="bg-white pa-2 border ma-2" elevation="4" style="border-radius: 10px;">
    <div class="d-flex justify-start align-center text-start ga-3">
        ...
        <div class="text-body">
          <div class="font-weight-black ">{{ props.menu.restaurant?.partner ? props.menu.restaurant?.partner.partner_name : '' }}</div>
          <div class="text-blue-grey-lighten-2 text-subtitle-2">{{ props.menu.restaurant?.partner?.address ? props.menu.restaurant?.partner.address : 'Marine Parade' }}</div>
        </div>
